...
export default function MatchPage() {
  ...
  const updateMatchData = useCallback((updatedMatch: Match) => {
    ...
    try {
      localStorage.setItem('futsal_match', JSON.stringify(updatedMatch));
    } catch (error) {
  ...
  useEffect(() => {
    const loadMatch = () => {
        try {
            const savedMatchJSON = localStorage.getItem('futsal_match');
            if (savedMatchJSON) {
                setMatch(JSON.parse(savedMatchJSON));
            } else {
                setMatch(initialMatch);
            }
        } catch (error) {
            console.error("Failed to parse match from localStorage", error);
            setMatch(initialMatch);
        }
    };
