...
export async function managePlayerAccess(playerId: string, newEmail: string, newPassword?: string): Promise<{ success: boolean, error?: string }> {
    const supabaseAdmin = createAdminClient();
    ...
    if (fetchError) {
        return { success: false, error: "Impossible de trouver le joueur." };
    }

    let authUserId = player.user_id;

    // --- Case 1: Player has no account, create one ---
    if (!authUserId) {
        if (!newPassword || newPassword.length < 6) {
            return { success: false, error: "Un mot de passe d'au moins 6 caractères est requis pour créer un compte." };
        }
        ...
        if (createError) {
            return { success: false, error: `Erreur de création de compte : ${createError.message}` };
        }
        authUserId = authData.user.id;
    }
    // --- Case 2: Player has an account, update it ---
    else {
        const updates: any = {};
        ...
        if (Object.keys(updates).length > 0) {
            const { error: updateError } = await supabaseAdmin.auth.admin.updateUserById(authUserId, updates);
            if (updateError) {
                return { success: false, error: `Erreur de mise à jour du compte : ${updateError.message}` };
            }
        }
    }
    
    // --- Finally, update the player's profile in the `players` table ---
    const { error: playerUpdateError } = await supabaseAdmin
        .from('players')
        .update({ user_id: authUserId, email: newEmail })
        .eq('id', playerId);

    if (playerUpdateError) {
        // This is problematic, but the auth user was already changed. We should log this.
        console.error(`Auth user was updated, but failed to update player profile ${playerId}:`, playerUpdateError);
        return { success: false, error: "Le compte a été mis à jour, mais une erreur est survenue sur le profil du joueur." };
    }
