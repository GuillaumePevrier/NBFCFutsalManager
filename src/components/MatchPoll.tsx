...
export default function MatchPollComponent({ poll, allPlayers, onPollChange, role }: MatchPollProps) {
        ...
        <Popover key={playerId}>
          ...
          <PopoverContent className="w-auto p-2">
             <div className="flex gap-2">
              <Button size="sm" variant="ghost" className="flex-1 bg-green-500/10 text-green-500 hover:bg-green-500/20 hover:text-green-500" onClick={() => handlePlayerResponse(playerId, 'available')}>
                <Check className="mr-2 h-4 w-4" /> Présent
              </Button>
              <Button size="sm" variant="ghost" className="flex-1 bg-red-500/10 text-red-500 hover:bg-red-500/20 hover:text-red-500" onClick={() => handlePlayerResponse(playerId, 'unavailable')}>
  ...
  if (role !== 'coach' && poll.status === 'inactive') return null;
