...
import { Clock, Play, PowerOff, RefreshCw, ThumbsDown, ThumbsUp, UserCheck, UserX, HelpCircle, Users } from 'lucide-react';
        ...
        <div key={playerId} className="flex items-center justify-between p-2 rounded-md bg-muted">
          ...
          {role === 'coach' && (
            <div className="flex gap-1">
              <Button size="icon" variant="ghost" className="h-8 w-8 text-green-500 hover:text-green-500 hover:bg-green-500/10" onClick={() => onPlayerResponse(playerId, 'available')}>
                <ThumbsUp className="h-4 w-4" />
              </Button>
              <Button size="icon" variant="ghost" className="h-8 w-8 text-red-500 hover:text-red-500 hover:bg-red-500/10" onClick={() => onPlayerResponse(playerId, 'unavailable')}>
